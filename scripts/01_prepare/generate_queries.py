"""各候補単語 × chunk から「サポートデスク問い合わせ風」query を LLM で生成.

入力: direction_a_candidates.json
出力: generated_queries.json

各候補について 2 query 生成 (バリエーション確保)。query は 15-40 文字、当該単語を必ず含む、
chunk 内容に直接対応する自然な検索意図。
"""
...
CANDIDATES_IN = DATA_DIR / "direction_a_candidates.json"
QUERIES_OUT = DATA_DIR / "generated_queries.json"
...
PROMPT_TMPL = """あなたは「{tenant_label}」業種に属する企業のサポートデスク担当者です。社内ナレッジベースから以下のドキュメントを探したい場面を想定してください。
